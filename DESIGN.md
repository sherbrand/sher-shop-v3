...
- **Breadcrumb** — page trail (Home › Shop); `eyebrow` type, `textMeta` links. Always one line, never wraps. The measure caps the whole trail at `35ch`, not each crumb, so a short parent leaves more room for the current label; the row also never outgrows a narrower placement. Parent crumbs and separators hold their full width and the current crumb ellipsis-trims, because the heading below repeats the current crumb while the parent is the trail's only link. A trail past the measure trims even where the placement has room, since a long trail reads as clutter. Props: `items`, `separator`.
