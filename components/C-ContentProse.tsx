...
/* C-ContentProse — the policy page band (shipping & returns, privacy, terms): a
   ...
   Stack several and they read as one document: the band opens flush at the top,
   a band followed by another closes flush too, and only the last of a run keeps
   its full bottom clearance. Each band after the first spaces itself inside. */
...
export interface ContentProseProps {
  ...
  background?: string;
  /** Add top padding back. The band opens flush by default. */
...
export function ContentProse({
  ...
  return (
    <section
      /* The marker is what lets one band see the band next to it. */
      data-prose-band
      className={[
        "@container px-[var(--gutter)]",
        // Opens flush; a band after a band gets its own breathing room back.
        "pt-0 [[data-prose-band]+&]:pt-[var(--space-6)]",
        // Only the last of a run keeps the full bottom clearance.
        "pb-[var(--space-9)] [&:has(+[data-prose-band])]:pb-0",
        className,
      ].join(" ")}
      style={{
