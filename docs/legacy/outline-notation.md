...
- **What a brief holds.** The subject, plus any fact the copy must contain. It can be as loose as a mood to hit.
...
- **Variable.** `{ }` marks a value that data fills in. It works in a notation line, in a copy bullet, and in frontmatter. The case you type is the case that renders: `{Category}` gives "Corset Tops", `{category}` gives "corset tops".
- **Where copy lives.** In the Planning TSV outline, a text slot carries its copy inline — a literal in quotes or a `< >` brief. In a Content MD, the slot is a bare role marker and its copy sits in a bullet beneath the line. `< >` stays in the line either way; it marks a target, a data-fed source, or a build-note, never copy. Asset slots may skip the brief. A bullet may carry `{ }`: a sample, or a variable.
- **Data-fed slot.** A slot whose content comes from a source. In a Content MD its bullet holds a `{ }` sample, and that `{ }` is what marks it. The line may also name the source with `<D-xxx: …>` (a Data resource) or `<F-xxx: …>` (a Feature), or leave it bare (`Title <product name>`) when the source is obvious. That flag is a hint, not required.
...
| `{ }` | a variable (e.g. `{Color}`, `{product name}`), or a condition (e.g. `{Corsets only}`) — see Rules for each |
