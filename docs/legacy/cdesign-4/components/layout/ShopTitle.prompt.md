...
- `measure` caps the description's line length (default `78ch`).
- A filter item with its own `onClick` renders as a button; without one (and without a shared `onFilter`) it renders as a link to `href`.
