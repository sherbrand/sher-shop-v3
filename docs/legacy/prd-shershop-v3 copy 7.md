...
## 3. Features
### F-001 — Product Grid
- **What it does:** Shows a grid of products pulled from a Shopify collection. Reused on the Shop page, the three category pages, and the "You May Also Like" block. Each product's thumbnail comes from the D-005 Product Data table: the picked image, or the first image if none is set.
...
### F-007 — Size Chart
- **What it does:** Renders a product's size chart from the D-005 Product Data table. Shows only the measurements the product defines and works out the inches table from the cm values.
...
| D-004 | Home Content | App repo config file | Hero banners, category tiles, and featured products (each as product name, URL, image, and link). Each slot gives placement, image, alt text, overlay text, and link. |
| D-005 | Product Data | App repo config file | Per-product data not in Shopify, as a TSV, one row per product keyed by slug. Columns: which image the grid uses as its thumbnail (falls back to the first image when blank), and cm size measurements as size columns (bust_S, waist_S, …). Inches are worked out from the cm values. |
...
| B-007 | Build the size chart drawer (from the Product Data table) and the shipping drawer. | C-Sizing, C-Shipping, F-007, D-005, /docs/content/s-012_shipping-returns.md |
...
| B-010 | Add SEO: per-page metadata, canonical URLs, JSON-LD Product structured data, the sitemap, and robots. | F-009, all screens, Extra Details |
