...
/* C-Shipping — the shipping & returns drawer, opened from the product page. Shows
   the same content as the /shipping-returns page. Pass your own content as
   children, or use the default sections. */
...
export function Shipping({
  ...
  const sectionLevel = Math.min(4, headingLevel + 1) as HeadingLevel;
          ...
          {children ??
            sections.map((section) => (
              <section key={section.title} className="mb-[var(--space-6)]">
                <Heading
                  level={sectionLevel}
                  className="m-0 mb-[var(--space-2)] font-[family-name:var(--font-nav)] text-[length:var(--text-sub)] font-normal uppercase leading-[var(--leading-snug)] tracking-[var(--tracking-display)] text-[var(--text-strong)]"
                >
                  {section.title}
                </Heading>
                <p className="m-0 text-[length:var(--text-sm)] leading-[var(--leading-normal)] text-[var(--text-default)]">
                  {section.body}
                </p>
              </section>
            ))}
