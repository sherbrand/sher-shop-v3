...
/* C-MediaGallery — the product page's media gallery: a thumbnail strip beside a
   ...
   The strip turns vertical at the GALLERY's own 380px via a container query.
   Nested in C-ProductPanel's two-up grid it gets about half the frame, so a
   page-level breakpoint would never fire. */
  ...
  return (
    <div className={`@container ${className}`}>
      <div className="flex flex-col gap-[var(--space-3)] @min-[380px]:grid @min-[380px]:grid-cols-[72px_minmax(0,1fr)]">
        ...
        >
          {media.map((item, i) => (
            <button
              ...
              onClick={() => setActive(i)}
              className={[
                "relative aspect-[3/4] w-[64px] shrink-0 cursor-pointer overflow-hidden p-0",
                "bg-[var(--surface-raised)] border transition-colors duration-[var(--dur-fast)] ease-[var(--ease-out)]",
                "@min-[380px]:w-[72px]",
            ...
            >
              {item.thumb ||
                ...
                (item.src && item.type !== "video" ? (
                  // eslint-disable-next-line @next/next/no-img-element -- placement passes a next/image via `thumb`; `src` is the plain fallback
        ...
        <div className="relative order-1 aspect-[3/4] overflow-hidden bg-[var(--surface-raised)] @min-[380px]:order-2">
          {media.map((item, i) => (
            <div
              key={i}
