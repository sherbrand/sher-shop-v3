...
export interface AccordionProps {
  items?: AccordionItem[];
  /** HTML level (h1–h4) for each row label — tag only, not style. Default 3. */
          ...
          <div key={`${item.q}-${i}`} className="border-b border-[var(--border-default)]">
            <Heading level={headingLevel} className="m-0">
              <button
                ...
                className="flex w-full cursor-pointer items-center justify-between gap-[var(--space-4)] border-none bg-transparent py-[var(--space-5)] text-left font-[family-name:var(--font-display)] text-[length:var(--fs-sub,var(--text-sub))] uppercase leading-[var(--leading-snug)] tracking-[var(--tracking-display)] text-[var(--text-strong)]"
              >
                <span>{item.q}</span>
                <span
            ...
            >
              <p className="m-0 max-w-[72ch] pb-[var(--space-5)] text-[length:var(--fs-body,var(--text-body-lg))] leading-[var(--leading-normal)] text-[var(--text-default)]">
                {item.a}
              </p>
            </div>
          </div>
