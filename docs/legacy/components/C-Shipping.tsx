    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-[var(--space-5)]">
          {children ||
            sections.map((s) => (
              <section key={s.title} className="mb-[var(--space-6)]">
                <Heading
                  level={Math.min(4, headingLevel + 1) as 1 | 2 | 3 | 4}
                  className="mb-[var(--space-2)] font-[family-name:var(--font-nav)] text-[length:var(--text-h4)] font-normal uppercase tracking-[var(--tracking-2)] text-[var(--text-strong)]"
                >
                  {s.title}
                </Heading>
                <p className="m-0 text-[length:var(--text-sm)] leading-[var(--leading-normal)] text-[var(--text-body)]">
                  {s.body}
                </p>
              </section>
            ))}
