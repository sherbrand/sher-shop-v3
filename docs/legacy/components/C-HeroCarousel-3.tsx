...
/* C-HeroCarousel — the Home hero band. A sliding "peek" carousel: one banner
   ...
   (2:3 on mobile/tablet, 4:5 on desktop). Auto-advances by one banner with wrap;
   prev/next arrows and dots page by one. Each banner takes a solid tone (bg) or an
   image, plus an optional text overlay (eyebrow/heading/cta) over a scrim. */
...
export function HeroCarousel({
  ...
  /* The CELL WIDTH is a container query (1-up below 768px, 2-up above); the track
    ...
    >
      <div
        ...
        className={[
          ...
          animate ? "transition-transform duration-[var(--dur-slow)] ease-[var(--ease-out)]" : "",
        ].join(" ")}
      ...
      >
        {extended.map((slide, k) => (
          <div
            key={k}
            className="relative aspect-[2/3] shrink-0 grow-0 basis-full overflow-hidden @min-[768px]:basis-1/2 @min-[1024px]:aspect-[4/5]"
          >
            <div
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-end gap-[var(--space-4)] px-[var(--gutter)] pb-[var(--space-9)] pt-[var(--space-8)] text-center text-[var(--sher-white)]">
                ...
                {slide.cta && (
                  <Button
                    ...
                    href={slide.cta.href}
                    variant="primary"
                    className="mt-[var(--space-2)] bg-[var(--sher-white)] text-[var(--sher-dark)] hover:bg-[var(--surface-tint)]"
                  >
                    {slide.cta.label}
                  </Button>
                )}
