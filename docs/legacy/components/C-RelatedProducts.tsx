...
/* C-RelatedProducts — the "You May Also Like" band: a heading, subtitle and an
   ...
   the Back to Category button plus any `children`, side by side or one per row
   (`actionsLayout`), with the back button's fill set by `backVariant`. Stacked,
   one `actionsMeasure` cap on the row keeps every button the same width. The band
   stacks below 768px of its own width. */
...
export interface RelatedProductsProps {
  ...
  /** Columns as "mobile/tablet/desktop". Default "1/2/2". */
  columns?: string;
  /** Fill for the Back to Category button. Default "secondary". */
  backVariant?: "primary" | "accent" | "secondary" | "ghost";
  /** "row" (default) sits the buttons side by side; "stack" gives each its own row. */
    ...
    <div className={`@container ${className}`}>
      <div className="grid grid-cols-1 items-start gap-[var(--space-5)] @min-[768px]:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] @min-[768px]:gap-[var(--space-8)]">
        <div className="flex flex-col items-start gap-[var(--space-3)] @min-[768px]:self-center">
          ...
          {hasActions && (
            <div
              /* "stack" puts one button per row, each filling the text column, and
              ...
              style={stack ? { maxWidth: actionsMeasure } : undefined}
            >
              {backLabel && backHref && (
