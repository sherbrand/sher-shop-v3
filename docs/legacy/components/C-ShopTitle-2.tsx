...
/* C-ShopTitle — the page header band for Shop and category pages: a breadcrumb,
   a heading, an optional description, and an optional row of filter pills.
   Heading and description sizes step against the band's OWN width via container
   queries — no page CSS, no JS measurement. */
...
export interface FilterPill {
  ...
  key?: string;
  active?: boolean;
  /** Per-pill handler — makes this pill a button instead of a link. */
  onClick?: () => void;
}
...
export function ShopTitle({
  ...
  return (
    ...
    >
      ...
      <Heading level={headingLevel} className={`m-0 ${HEADING_FACE} ${STEP_HERO}`}>
        {heading}
      </Heading>
      {description && (
        <p
      ...
      {filters && filters.length > 0 && (
        ...
        >
          {filters.map((filter, i) => {
            const active = filter.active ?? (activeFilter != null && filter.key === activeFilter);
            ...
            const handler =
              filter.onClick ?? (onFilter && filter.key ? () => onFilter(filter.key as string) : undefined);
            return handler ? (
              <ButtonPill key={key} active={active} onClick={handler}>
                {filter.label}
              </ButtonPill>
            ) : (
