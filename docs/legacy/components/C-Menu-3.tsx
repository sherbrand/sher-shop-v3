...
/* C-Menu — Menu Drawer.
   ...
   group, then Our Story / Contact Us / Login. A link closes the drawer and navigates. */
...
const LINK =
  "block py-[var(--space-2)] font-[family-name:var(--font-nav)] uppercase tracking-[var(--tracking-display)] text-[var(--text-strong)] no-underline";
...
export function Menu({
  ...
  const go = (href: string) => (event: MouseEvent): void => {
    if (onNavigate) {
    ...
    >
      ...
      >
        ...
        <div className="mb-[var(--space-6)] flex flex-col">
          {shopLinks.map((link) => (
            <a key={link.href} href={link.href} onClick={go(link.href)} className={`${LINK} ${STEP_TITLE}`}>
              {link.label}
            </a>
          ))}
        </div>
        ...
        <div className="mt-auto flex flex-col gap-[var(--space-1)] border-t border-[var(--border-default)] pt-[var(--space-5)]">
          {secondaryLinks.map((link) => (
            <a
              ...
              className="py-[var(--pad-row-md)] font-[family-name:var(--font-body)] text-[length:var(--size-base)] tracking-[var(--tracking-item)] text-[var(--text-default)] no-underline"
            >
              {link.label}
            </a>
          ))}
