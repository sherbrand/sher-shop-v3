...
/* C-Sticky — the sticky header shown on every screen (and taking over the Home
   hero after 60vh). Solid page background with a hairline base. Dark monogram
   mark centered; hamburger opens C-Menu, cart icon opens C-Cart. */
    ...
    >
      ...
      <div className="px-[var(--gutter)]">
        <div className="relative mx-auto flex h-[var(--header-h)] max-w-[var(--container)] items-center justify-between">
          {/* optical hedge: the 44px IconButton centers a ~26px glyph, so pull it
              back to align the GLYPH with the page gutter, not the button box */}
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute right-[2px] top-[4px] h-[16px] min-w-[16px] px-[4px] text-center font-[family-name:var(--font-body)] text-[length:var(--text-nano)] leading-[16px] tabular-nums rounded-[var(--radius-pill)] bg-[var(--surface-inverse)] text-[var(--text-on-inverse)]">
                {cartCount}
              </span>
            )}
          </IconButton>
