...
/* C-Cart — the cart drawer. Slides from the right. Line items (image, name,
   options, stepper, price, remove), a subtotal, and the Shopify checkout handoff.
            ...
            {items.map((item) => (
              <div key={item.id} className="flex gap-[var(--space-4)]">
                <div className="h-[96px] w-[72px] shrink-0 overflow-hidden bg-[var(--surface-raised)] rounded-[var(--radius-sm)]">
                  {item.image && (
                    // eslint-disable-next-line @next/next/no-img-element -- cart line thumb comes from the Shopify CDN, already sized
                    <img
                      src={item.image}
                      alt={item.name}
