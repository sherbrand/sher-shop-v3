    ...
    >
      ...
      {pageSize && products.length > 0 ? (
        remaining > 0 ? (
          ...
          // End of the collection: a short centered rule terminates the grid.
          <hr
            aria-hidden="true"
            className="mx-auto mb-0 mt-[var(--space-8)] w-[64px] border-0 border-t border-t-[var(--border-strong)]"
          />
        ) : endMark === "mark" ? (
          // End of the collection: a diamond knocked out of a tapered hairline.
          <div aria-hidden="true" className={`relative flex items-center justify-center ${END_MARK}`}>
            ...
            <span className="absolute h-[6px] w-[6px] rotate-45 bg-[var(--text-strong)] shadow-[0_0_0_4px_var(--surface-page)]" />
          </div>
        ) : endMark === "monogram" ? (
          // End of the collection: the SHER mark between two outward-fading rules.
          <div
            aria-hidden="true"
            className={`flex items-center justify-center gap-[var(--space-2)] ${END_MARK}`}
          >
