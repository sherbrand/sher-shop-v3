...
Type roles are named for WHERE the type sits, never for an HTML tag — the ladder is `display` -> `hero` -> `section` -> `title` -> `sub`, and any of them can render as any h1-h4 via the Heading component's `level` prop. Two stepped body roles carry non-heading text: **item** (`--text-item-sm/md/lg`, 16 / 18 / 20) for product names and prices, and **body** (`--text-body-sm/md/lg`, 14 / 16 / 18) for descriptions and intro paragraphs. Pages alias them as `--fs-item` and `--fs-body` and step them at container breakpoints. Fixed sizes remain for `--text-base` (16 prose), `--text-sm` (14 meta), `--text-xs` (12 labels), `--text-nano` (10 cart badge). The announcement bar keeps its own stepped role, `--text-announce-sm/lg` (10 / 11.5), separate from `--text-nano` so it can move independently. The announcement bar has its own stepped role, `--text-announce-sm/lg` (10 / 11.5), aliased as `--fs-announce`.
...
For container-driven pages, fixed **stepped** heading tokens (`--step-display|h1|h2|h3|label-sm|md|lg`) let a page alias a size on a wrapper and bump it at the system breakpoints via container queries — deliberate, controllable sizing without fluid `clamp` or viewport units.
...
- **ProductCard** — F-001 grid unit; media (3:4) over a `category` eyebrow + title on the left with the price right-aligned on the same row; `surfaceRaised`, `rounded.sm`. Props: `title`, `price`, `compareAt`, `category`, `href`, `src`/`media`, `soldOut`.
...
**Layout Components** (`components/layout/`) — every heading takes a `headingLevel` (h1–h4) prop that changes the tag only, not the style.
- **HeroCarousel** — Home hero band (F-008); sliding peek carousel, 1 banner mobile / 2 at 50% tablet+desktop, portrait crops (2:3 → 4:5). Auto-advances by one with wrap; arrows + dots. Props: `slides`, `interval`, `autoPlay`, `twoUpFrom`, `ratioMobile`, `ratioDesktop`, `desktopFrom`.
...
- **EditorialSplit** (C-EditorialSplit) — two-up editorial band used on the About page; 4:5 media beside eyebrow + heading + paragraph, `mirror` swaps sides, stacks on mobile. Props: `eyebrow`, `heading`, `headingLevel`, `paragraph`, `media`, `mirror`, `mobileFirst` ("media"|"text"), `mediaRounded`, `twoUpFrom`, `background`, `children`. Column gap is set via the `--editorial-gap` custom property so pages can step it per breakpoint.
- **ShopEditorial** (C-ShopEditorial) — the two-up editorial band used across the Shop and category pages; same shape as C-EditorialSplit. Props: `eyebrow`, `heading`, `headingLevel`, `headingFont` ("display"|"body"), `paragraph`, `media`, `mirror`, `mobileFirst` ("media"|"text"), `mobileAlign` ("left"|"right"), `twoUpFrom`, `gap`, `ratio`, `background`, `children`. The band steps its own gap (24 → 64px) and media crop (4:3 → 1:1 → 5:4) off its measured width, so pages need no CSS; `gap`/`ratio` or `--editorial-gap` override per placement.
- **FeatureColumns** (C-FeatureColumns) — eyebrow + heading over 2–3 media columns, each with a subheading and paragraph. Props: `eyebrow`, `heading`, `headingLevel`, `itemHeadingLevel`, `items`, `twoUpFrom`, `threeUpFrom`.
- **ShopFaq** (C-ShopFaq) — FAQ band; heading beside its accordion at two-up, stacking on mobile. Heading size and column gap step off its own measured width. Props: `heading`, `headingLevel`, `items`, `itemHeadingLevel`, `defaultOpen`, `single`, `twoUpFrom`, `align` ("center"|"start").
- **ProductPanel** (C-ProductPanel) — product page main band (S-006); media gallery beside the purchase column (breadcrumb, name, price, type attribute, size, quantity, buy buttons, drawer links). All sizes sold out swaps the buy pair for a Preorder link. Props: `breadcrumb`, `name`, `headingLevel`, `price`, `compareAt`, `description`, `attributeLabel`, `attributeValue`, `sizes`, `size`, `onSize`, `quantity`, `onQuantity`, `media`, `onAddToCart`, `onBuyNow`, `onSizeChart`, `onShipping`, `preorderHref`, `twoUpFrom`.
- **MediaGallery** (C-MediaGallery) — thumb strip beside a 3:4 stage; video first, autoplaying muted/looped/inline once loaded, static under `prefers-reduced-motion`. Props: `media`, `thumbsFrom`.
- **RelatedProducts** (C-RelatedProducts) — "You May Also Like" band; heading + subtitle + back-to-category button over a small ProductCard grid. Props: `heading`, `headingLevel`, `subtitle`, `backLabel`, `backHref`, `products`, `columns`, `backVariant`, `actionsLayout` ("row"|"stack"), `actionsMeasure`, `children` (extra buttons in the actions row).
...
- **CategoryGrid** (C-CategoryGrid) — full-bleed grid of category tiles; 1 col mobile / 2 tablet+desktop, portrait 4:5 crops, alternating bottom-left/right labels over a scrim. Props: `items`, `headingLevel`, `twoUpFrom`, `alternate`.
- **HeroTitle** — editorial title band beneath the hero; optional breadcrumb + eyebrow + `display-1` heading + `lead` description + optional actions. Props: `breadcrumb`, `eyebrow`, `heading`, `headingLevel`, `description`, `align` ("center"|"start"), `measure`, `headingFont` ("display"|"body"), `headingMeasure`, `background`, `children`.
- **Transparent** — hero header, Home only; `white` text over imagery, height 4.5rem. Props: `onMenu`, `onCart`, `cartCount`, `announcement`, `announcementTone` ("dark"|"light"|"accent").
- **Sticky** — sticky header on every screen; `surfacePage`/`textStrong`, height 4.5rem. Props: `onMenu`, `onCart`, `cartCount`, `announcement`, `announcementTone` ("dark"|"light"|"accent"), `showAnnouncement`.
- **Menu** — left menu drawer; `surfacePage`, padding 1.5rem. Props: `open`, `onClose`, `headingLevel`, `shopLinks`, `secondaryLinks`, `onNavigate`.
- **Cart** — right cart drawer; `surfacePage`, `h3` title, padding 1.5rem. Props: `open`, `onClose`, `headingLevel`, `items`, `onQuantityChange`, `onRemove`, `onCheckout`.
