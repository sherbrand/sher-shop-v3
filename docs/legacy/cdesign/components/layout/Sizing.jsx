...
function Table({ measures, rows, unit }) {
  const cell = {
    padding: "0.6rem 0.75rem", textAlign: "left", fontFamily: "var(--font-body)",
  ...
  const head = { ...cell, color: "var(--text-meta)", textTransform: "uppercase",
    letterSpacing: "var(--tracking-label)", fontSize: "var(--text-xs)" };
  return (
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between",
          ...
          <div>
            ...
            {productName && <p style={{ margin: "0.25rem 0 0", color: "var(--text-meta)",
              fontSize: "var(--text-sm)" }}>{productName}</p>}
          </div>
          <IconButton label="Close size chart" onClick={onClose}><Icon name="close" size={24} /></IconButton>
        </div>

        <div style={{ flex: 1, overflowY: "auto", padding: "var(--space-5)" }}>
          <p style={{ marginTop: 0, color: "var(--text-default)", fontSize: "var(--text-sm)",
