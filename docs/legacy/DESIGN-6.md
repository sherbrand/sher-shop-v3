...
Semantic aliases reference the raw palette: `surfacePage`/`surfaceTint`/`surfaceRaised`/`surfaceInverse`, `textDefault`/`textStrong`/`textMeta`/`textMuted`, `textOnInverse`, `textOnPrimary`, `borderDefault`, `borderFocus`. `accent` is listed once, in the raw palette, and referenced directly. Reference tokens with `{colors.<name>}`.
...
Four optional page overrides let a placement pin a size that would otherwise step: `--fs-item`, `--fs-body`, `--fs-sub`, and `--fs-announce`. Each component reads its override with the matching token as the fallback, so leaving them unset keeps the stepped behavior.
...
Breakpoints: 640 / 768 / 1024 / 1280 / 1536. Container max-width 1536px; wide media bands cap at the same 1536px (`--container-media`); readable prose measure 768px. The page edge padding (`--gutter`) is responsive, scaling with the frame: 1rem (16px) on mobile, 1.5rem (24px) on tablet, 3rem (48px) on desktop. The app layout sets it; every band reads it. Header height 4.5rem, announcement bar 2.25rem.
...
- **Heading** — level-to-tag primitive; `level` sets h1–h4 for the outline without changing style. Binds no tokens of its own. Props: `level`, `as`, `children`.
...
- **C-HeroTitle** — editorial title band beneath the hero; optional breadcrumb + eyebrow + `display-1` heading + lead description + optional actions. Props: `breadcrumb`, `eyebrow`, `heading`, `headingLevel`, `description`, `align` ("center"|"start"), `measure`, `headingFont` ("display"|"body"), `headingMeasure`, `background`, `tone` ("light"|"inverse"), `children`.
...
- **C-ProductGrid** — shop / category results band; count + ViewToggle toolbar over a responsive ProductCard grid. Columns come from one "mobile/tablet/desktop" string. A floating copy of the ViewToggle sticks to the scrollport bottom-left once the toolbar scrolls away. Closes with a Divider when every product is shown. Props: `products`, `columns`, `label`, `count`, `onView`, `showToolbar`, `floatingToggle`, `pageSize`, `loadMoreLabel`, `endMark` ("none"|"rule"|"mark"|"monogram"), `emptyMessage`.
...
- **C-Sizing** — right size-chart drawer; `surfacePage`, `h3` title, padding 1.5rem. Renders only the measures a product defines; the inches table derives from the cm values. Props: `open`, `onClose`, `headingLevel`, `productName`, `chart`.
