---
type: style-reference
brand: SHER
---

# Visual Style

This is the confirmed visual style that seeds the design system and its components. Pairs with the brand file — read both. It owns voice, positioning, and audience. For each section, and each part within a line: fill it, or write **None yet.** (still open — decide in real designs).

## 1. Look & Feel
Concept — the aesthetic in one sentence ("warm editorial, photography-led"). Personality and Resemblance are copied from the brand file; it is the source of truth.
- concept: None yet.
- personality: Magnetic, Sensual, Feminine, Refined
- resemblance: Cherie (sweet sensuality), Perfect Corset (bold sensuality), Gigi Hadid (off-duty model)

## 2. Color
One line per color: name, role ("base background", "body text", "interactive accent"), and hex. Name the set and never go outside it. Some are required; others optional ("white", "dark", "muted").

Required — always in the palette:
- background (base background): #FAF9F6
- surface (secondary background): #E4E2DC
- text (body text): #514F4E
- primary (interactive accent): #A99D94

Optional — add each only if the design uses it:
- muted: #929291
- dark: #333231

Approved pairs — which text sits on which background and passes WCAG AA ("text on background", "white on dark").
- None yet.

## 3. Typography
One line per role: name, use, typeface ("Georgia"), case ("Sentence case"), letter-spacing ("0.04em"), and size ("16/24/32/48px"). Some are required; others optional ("H3-H6", "title", "label", "nav", "buttons").

Required — always defined:
- display (H1-H2): Cormorant Infant (Uppercase, 0.05em, None yet)
- body (prose): Cardo (None yet)

Optional — add each only if the design uses it:
- nav (menus): Cormorant Infant (Uppercase, 0.05em, None yet)
- button (primary, secondary): Cormorant Infant (Uppercase, 0.05em, None yet)
- label (eyebrow, breadcrumb): Cardo (Title Case, 0.02em, None yet)

## 4. Logo
One line per variation ("wordmark", "monogram") with its colors, then the clear space ("one cap-height") and misuses to ban ("never stretch or recolour").

Variations:
- monogram (mark alone): black, white, dark gray
- square logo (monogram + wordmark below): black, white

Clear space:
- None yet.

Don'ts:
- None yet.

## 5. Imagery
What the photos show and how they look — location ("studio, city street"), palette ("earth tones"), and lighting ("golden hour").
- location: architectural, lived-in (e.g. marble, staircases, balconies); plus one more (exploring)
- palette: nude, black, white; warm neutrals (taupe, beige, sage)
- lighting: None yet.

## 6. Iconography
Two jobs. UI icons carry the look. Brand marks are official brand logos.

UI icons — library ("Lucide"), fill ("line"), stroke ("1.5px"), and corners ("rounded").
- library: Lucide
- fill: line
- stroke: 1.5px
- corners: None yet.

Brand marks — the set ("Simple Icons"), for official logos only (payment, social).
- library: Simple Icons

## 7. Layout & Grid
Breakpoints in px ("640 / 768 / 1024"), container widths + padding ("content — 1200 / 24"), and the spacing rhythm ("generous, 4px base").
- breakpoints: 640 / 768 / 1024 / 1280 / 1536
- containers: None yet.
- spacing rhythm: None yet.

## 8. Shape & Depth
Corner-radius character ("small / 4px"), flat vs layered ("flat"), and shadow philosophy ("depth from tone, not shadow").
- radius: small
- layering: None yet.
- shadow: None yet.

## 9. Motion
Speed ("quick"), easing ("ease-out"), and motion to ban ("no bounce or overshoot").
- speed: None yet.
- easing: None yet.
- avoid: None yet.

## 10. Do's & Don'ts
Approved moves to encourage ("full-bleed imagery"), and anti-patterns to ban, including generic-AI tells ("no gradients", "no drop shadows").

Do's:
- None yet.

Don'ts:
- None yet.
