---
title: "{product name} | SHER"
description: "Shop the {product name} by SHER. Built on real corsetry, with every bone set by hand on a mannequin, so the shape holds to a real body."
url: /products/[product-slug]
---

`S-006.1: Media Gallery <thumbnail strip + main image; fixed order: video then images> | (Breadcrumb <{Category} (to /{category}) › product; Shop › product where the product has no category> / Title <product name> / Price / Size Selector <available sizes> / Btn.1 / Btn.2 / (Link.1 <opens C-Details> + Link.2 <opens C-Sizing> + Link.3 <opens C-Shipping>))`
- Title: {Silk Lace Corset Top}
- Btn.1: Add to Cart
- Btn.2: Buy Now
- Link.1: Details
- Link.2: Sizing
- Link.3: Shipping & Returns

`S-006.2: Divider // (H2 / Subtitle) // Product Grid <4 products from this and other categories, in a swipe rail>`
- H2: You May Also Like
- Subtitle: Browse popular pieces from this and other categories.
