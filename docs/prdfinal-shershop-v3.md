...
### F-001 — Product Grid
- **What it does:** Shows a grid of products from a Shopify collection. Each card shows the media D-005 Product Data picks, and moves to another on hover. Both are positions, counted from 1 the way the Shopify admin counts them, with the video counted. A product with no row, a number past the end, or a number that lands on the video shows the product's first image instead. Hovering a card on desktop, or holding a finger on it on tablet and mobile, moves the card to its hover media. A video there plays; an image just swaps in. Where D-005 leaves the hover blank, the card takes the first media that is not its own. It goes back when the pointer leaves or the finger lifts. Use `mouseenter` and `mouseleave` on desktop, and `touchstart` and `touchend` on tablet and mobile. The video plays muted, looped and inline, and loads nothing until the pointer arrives. A browser that cannot play it shows that video's own still frame instead. A product with nothing else to show does not swap. On the listing screens the grid fetches the whole collection server-side, shows 12, and a Load More button reveals the next 12. Load More keeps any active filter and column choice. A divider sits under the grid when nothing is left.
...
| D-005 | Product Data | `/data/d-005_product-data.tsv` | Per-product data that is not in Shopify. One row per product, keyed by slug. Columns: which media the grid card shows (`thumb`), which it moves to on hover (`hover`), and the inch measurements as one column per size (`bust_S`, `bust_M`, …). `thumb` and `hover` are positions in the product's media, counted from 1 the way the Shopify admin counts them, with the video counted. A blank `hover` takes the first media that is not the card's own. A measurement can be a single number, a range like `30-31`, `FS` for free size, or two numbers like `9 / 14` where a set has a separate top and bottom length. A product fills only the measurements it uses and leaves the rest blank. Centimeters are worked out from the inch values. |
