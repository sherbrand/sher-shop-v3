...
/* Home page (S-001, B-008): the hero band (S-001.1), the intro title (S-001.2),
   the category tiles (S-001.3), and the featured rail (S-001.4), which carries
   the page's closing CTA. Sections set their own width, so <main> is full-bleed.
...
export default async function HomePage(): Promise<ReactElement> {
  ...
  return (
    <main>
      ...
      {/* S-001.4 — the featured rail, and the page's closing call to action in
      ...
      {featured.length > 0 && (
        ...
        >
          ...
          <ProductCarousel
            ...
            headingLevel={2}
            subtitle={slotText("s-001.4.subtitle")}
            peek="1.5/2.5/3.5"
            products={featured}
